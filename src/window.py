...
class Window(object):
    ...
    def change_by_downloaded_settings(
        self, cbx_ai_model, cbx_language, cbx_view_format, prg_settings_selected
    ):
    ...
    def set_widgets(self, ofr_requests, ofr_settings):
        ...
        leb_selected_promt_file = Label(
            ifr_ai_model, text=settings.get_settings()["promt_file"]
        )
        leb_selected_promt_file.pack(**label_pack_params)

        btn_choose_promt_file = Button(
            ifr_ai_model,
            text=get_tr()["search_in_filesystem"],
            command=self.select_promt_file,
        )
        btn_choose_promt_file.pack(**widget_pack_params)

        leb_entry_api_key = Label(ifr_ai_model, text=get_tr()["ask_entry_api_key"])
        ...
        leb_select_language = Label(ifr_other, text=get_tr()["ask_select_language"])
        ...
        leb_select_view_format = Label(ifr_other, text=get_tr()["ask_select_format"])
        ...
        btn_docks = Button(
            ifr_other, text=get_tr()["read_docks"], command=Window.open_repo_docks
        )
        ...
        leb_entry_source_text = Label(
            ifr_source_text, text=get_tr()["ask_entry_source_text"]
        )
        ...
        edt_ai_answer = tkhtmlview.HTMLScrolledText(
            ifr_ai_answer, html="", **text_init_params
        )
        ...
        btn_save_answer_to_clipboard = Button(
            ...
            command=lambda: self.save_to_clipboard(
                ai_request.read_ais_response_from_file()
            ),
        )
        btn_save_answer_to_clipboard.pack(**widget_pack_params)

        self.change_by_downloaded_settings(
            cbx_ai_model, cbx_language, cbx_view_format, prg_settings_selected
        )

        btn_ask_ai = Button(
            ifr_ask_ai,
            text=get_tr()["btn_ai_question"],
            command=lambda: ai_request.get_ai_response(
                edt_source_text.get("1.0", END),
                edt_essay_text.get("1.0", END),
                edt_ai_answer,
            ),
        )
